...
cat $data/doc*.ba-s > $out_dir/bleualign/align.ba-s
cat $data/doc*.ba-t > $out_dir/bleualign/align.ba-t
rm $data/doc*.ba-{s,t}


# Bleualign (both directions):
mkdir -p $out_dir/bleualign2/
for doc in `ls $data/doc*_zh.snt`; do
	$bleualign --factored \
	-s $doc \
	-t ${doc/_zh/_en} \
...
cat $data/doc*.ba2-s > $out_dir/bleualign/align.ba2-s
cat $data/doc*.ba2-t > $out_dir/bleualign/align.ba2-t
rm $data/doc*.ba2-{s,t}


# Gale-Church:
mkdir -p $out_dir/galechurch/
for doc in `ls $data/doc*_zh.snt`; do
	$bleualign --factored \
	-s $doc \
	-t ${doc/_zh/_en} \
...
cat $data/doc*.gc-s > $out_dir/galechurch/align.gc-s
cat $data/doc*.gc-t > $out_dir/galechurch/align.gc-t
rm $data/doc*.gc-{s,t}
